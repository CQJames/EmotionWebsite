$(function(){
	for(i=0;i<2;i++){
		$("#commentApplaud"+i).click(function(){
			 ...
			 var url = 'SelfEnhancementCommentAjaxAction_addApplaud?selfEnhancementCommentID=' + dataid;
			 $.ajax({
	             url: url,
		         type: 'get',
		...
		$("#commentDisapproval"+i).click(function(){
			 var span = $(this).children().eq(1);
			 var dataid = span.attr("data-id");
			 var url = 'SelfEnhancementCommentAjaxAction_addDisapproval?selfEnhancementCommentID=' + dataid;
			 $.ajax({
	             url: url,
		         type: 'get',
			     success: function (data) {
	...
	$('#btnSubmit').click(function(){
		var selfEnhancementArticleID = $('#selfEnhancementArticleID').html();
		var message = $(this).parent().prev().children().val();
		$.ajax({
            url:"SelfEnhancementCommentAjaxAction_publishComment",
		    type:"post",
		    data:{"selfEnhancementArticleID":selfEnhancementArticleID,"message":message},
		    success:function(){
		    	//成功后刷新评论,回到第一页,且数据加1
