$(function() {
	$('#dg').datagrid({
		url: 'SelfEnhancementArticleBackstageAction_findAllExcludeComment',
		columns: [[  
			  ...
			  {field: 'publisher',title: '发布者',width: '5%',align:'center'},
			  ...
			  {field: 'xxx',title: '操作',width: '15%',align:'center',formatter: function(value, row, index) {
					return "<a href='javascript:void(0)' class='loan_update_button' onclick=\"edit('" + row.selfEnhancementArticleID + "')\">" + "修改</a>" +
							"| <a href='javascript:void(0)' class='loan_delete_button' onclick=\"del('" + row.selfEnhancementArticleID + "')\">" + "删除</a>"+
							"| <a href='javascript:void(0)' class='loan_redo_button' onclick=\"viewComment('" + row.selfEnhancementArticleID + "')\">" + "查看评论</a>";
					}
				}
			]],
	...
	$('#dgComment').datagrid({
		url: 'SelfEnhancementCommentBackstageAction_findComment',
		columns: [[  
			       ...
			       {field: 'xxx',title: '操作',width: '10%',align:'center',formatter: function(value, row, index) {
						return "<a href='javascript:void(0)' class='loan_delete_button' onclick=\"Commentdel('" + row.selfEnhancementCommentID + "')\">" + "删除</a>";
						}
				   }
				]],			
		onLoadSuccess: function(data) {
	...
	$('#update_category').combobox({
		onChange: function(){	
			var category = $('#update_category').combobox('getValue');
				 if(category === "认识自我") {$('#category').val("认识自我");} 
			else if(category === "自身建设") {$('#category').val("自身建设");}
		}
	});
});
...
function save() {
	// 提交数据到Action
	$('#formAdd').form('submit', {
		url: "SelfEnhancementArticleBackstageAction_save",
...
function selectradio() {
	$("#saveTime").val(new Date($("#saveTime").val()).Format("yyyy-MM-dd hh:mm:ss"));
		 ...
		 if($("#category").val() === "认识自我"){$('#update_category').combobox('select', '认识自我');}
	else if($("#category").val() === "自身建设"){$('#update_category').combobox('select', '自身建设');}

		 
	if($("#isDisplay").val() === "true"){$('#update_isDisplay').combobox('select', '是');}
	else{$('#update_isDisplay').combobox('select', '否');}
...
function edit(id) {
	...
	$('#formUpdate').form('load', "SelfEnhancementArticleBackstageAction_findByIdExcludeComment.action?selfEnhancementArticleID=" + id + "");
...
function update() {
	$('#formUpdate').form('submit', {
		url: "SelfEnhancementArticleBackstageAction_update",
		success: function(data) {
			var jsonData = eval("(" + data + ")");
			$.messager.show({
				title: '提示消息',
...
function del(id) {
	$.messager.confirm('确认', '您确认想要删除记录吗？', function(r) {
		if(r) {
			$.post("SelfEnhancementArticleBackstageAction_delete", {
				"selfEnhancementArticleID": id
			}, function(data) {
				$.messager.show({
					title: '提示消息',
					msg: data.msg,
...
function formatIcon(value,row){
	...
	str = '<img style="height:60px; width:150px;" src=".'+value+'" onclick="viewImg(\''+value+'\');" class="easyui-tooltip=" id="'+row.selfEnhancementArticleID+'" title="点击预览图片"/>';
    return str;
}
		...
		if(confirm) {
			if(operate == 'add'){
				$("#iconImage_upload_add").form('submit',{
					url:'SelfEnhancementArticleBackstageAction_upLoad',
					success:function(data){
						 var res = eval ("(" + data + ")");
...
function formatWzlx(value){
	     if (value === "认识自我") return "认识自我";
	else if (value === "自身建设") return "自身建设";
}
