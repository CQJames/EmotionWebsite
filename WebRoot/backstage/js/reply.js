$(function() {
	$('#dg').datagrid({
		url: 'ReplyBackstageAction_findAllPage',
		columns: [[  
		      {field: 'message',title: '内容',width: '30%',align:'center'},
		      {field: 'utterance',title: '评论的问题',width: '30%',align:'center', formatter:function(val){if(val){return  val.message}},},
			  ...
			  {field: 'xxx',title: '操作',width: '10%',align:'center',formatter: function(value, row, index) {
					return "<a href='javascript:void(0)' class='loan_delete_button' onclick=\"del('" + row.replyID + "')\">" + "删除</a>";
					}
				}
			]],
...
function searchGrid(){
	$('#dg').datagrid('reload', {
		searchName: $('#reply-message').textbox('getValue'),
	});
